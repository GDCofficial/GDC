...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gdc is "gdc". If no
...
// These resources are resolved differently for "gdc" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gdc 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				...
				log.Warn("Using deprecated resource file, please move this file to the 'gdc' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
