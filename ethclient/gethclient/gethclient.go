...
// Package gethclient provides an RPC client for gdc-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements gdc-specific functionality.
//
...
// GCStats retrieves the current garbage collection stats from a gdc node.
...
// MemStats retrieves the current memory stats from a gdc node.
...
// GetNodeInfo retrieves the node info of a gdc node.
