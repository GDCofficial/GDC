...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gdc".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gdc %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gdc %s", params.VersionWithMeta)
	}
