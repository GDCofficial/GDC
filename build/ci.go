...
var (
	// Files that end up in the gdc*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gdc"),
	}

	// Files that end up in the gdc-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	...
	var (
		...
		geth     = "gdc-" + basegeth + ext
		alltools = "gdc-alltools-" + basegeth + ext
	)
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gdc-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gdc-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gdc-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the gdc binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gdc.nsi", filepath.Join(*workdir, "gdc.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("gdc-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
