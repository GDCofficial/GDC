...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	ipcName := fmt.Sprintf("gdc-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	// We can't know exactly how long gdc will take to start, so we try 10
...
func initGeth(t *testing.T) string {
	...
	t.Logf("Initializing gdc: %v ", args)
	g := runGeth(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
