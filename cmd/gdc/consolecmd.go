...
See https://gdc.ethereum.org/docs/interacting-with-gdc/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gdc.ethereum.org/docs/interacting-with-gdc/javascript-console.
This command allows to open a console on a running gdc node.`,
	}
...
JavaScript API. See https://gdc.ethereum.org/docs/interacting-with-gdc/javascript-console`,
	}
)
...
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("failed to attach to the inproc gdc: %v", err)
	}
...
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gdc: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
